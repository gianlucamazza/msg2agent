...
export async function handleCallback(): Promise<boolean> {
  ...
  if (errCode) {
    t.pkceVerifier.set(null); t.oauthState.set(null);
...
export async function tryRefresh(): Promise<boolean> {
  ...
  if (!r.ok) {
    const err = await r.json().catch(() => ({}));
    if (err.error === 'invalid_client') t.clientId.set(null);
    t.accessToken.set(null); t.refreshToken.set(null);
    return false;
  }
  const data = await r.json();
  t.accessToken.set(data.access_token);
...
export function signOut(): void {
  ...
  if (refresh && cid) {
    fetch(AS() + '/oauth/revoke', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ token: refresh, token_type_hint: 'refresh_token', client_id: cid }),
    }).catch(() => {});
  }
